    ...
    if (!mechanical_interface_pending_pairs_.empty())
    {
        if constexpr (DisplacementDim == 2)
        {
            ...
            mechanical_interface_runtime_ = std::make_unique<
                ...
                std::move(runtime_pairs), mechanical_interface_materials_);
        }
        else
        {
            OGS_FATAL(
    ...
    std::map<int, std::shared_ptr<MaterialPropertyLib::Medium>> const& media)
{
    if (_use_monolithic_scheme)
    {
        initializeProcessBoundaryConditionsAndSourceTerms(
            *_local_to_global_index_map, 0, media);
        return;
    }
    initializeProcessBoundaryConditionsAndSourceTerms(
        *local_to_global_index_map_with_base_nodes_, 0, media);
    initializeProcessBoundaryConditionsAndSourceTerms(
        *_local_to_global_index_map, 1, media);
}
        ...
        GlobalVector& b, GlobalMatrix& Jac)
{
    ...
    // G5 is a mechanical interface only.  For monolithic RM the global vector
    // contains p and u, but the resolved G5 pair stores displacement DOF ids
    // only; for staggered RM this hook runs only for the mechanical process.
