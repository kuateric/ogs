      ...
      process_data_(std::move(process_data))
{
    ...
    ProcessLib::Reflection::addReflectedIntegrationPointWriters<DisplacementDim>(
        LocalAssemblerIF::getReflectionDataForOutput(), _integration_point_writer,
...
void RichardsMechanicsProcess<DisplacementDim>::constructDofTable()
{
    ...
    if (_use_monolithic_scheme)
    {
        ...
        std::vector<int> const n_components{1, DisplacementDim};
        _local_to_global_index_map = std::make_unique<NumLib::LocalToGlobalIndexMap>(std::move(subsets), n_components, NumLib::ComponentOrder::BY_LOCATION);
    ...
    else
    {
        ...
        std::vector<int> const n_components{DisplacementDim};
        _local_to_global_index_map = std::make_unique<NumLib::LocalToGlobalIndexMap>(std::move(subsets), n_components, NumLib::ComponentOrder::BY_LOCATION);
...
void RichardsMechanicsProcess<DisplacementDim>::initializeConcreteProcess(NumLib::LocalToGlobalIndexMap const& dof_table, MeshLib::Mesh const& mesh, unsigned const integration_order)
{
    ...
    if (!mechanical_interface_pending_pairs_.empty())
    {
        if constexpr (DisplacementDim == 2)
        {
            ...
            auto runtime_pairs = ProcessLib::MechanicalInterface::resolve2DPendingPairsToProcessDofs(mechanical_interface_pending_pairs_, mesh.getID(), dof_table, displacement_variable_id);
            mechanical_interface_runtime_ = std::make_unique<ProcessLib::MechanicalInterface::MechanicalInterfaceSmallDeformationRuntime>(std::move(runtime_pairs), mechanical_interface_materials_);
        }
        else
        {
    ...
    auto add_secondary_variable = [&](std::string const& name, int const num_components, auto f)
    {
        _secondary_variables.addSecondaryVariable(name, makeExtrapolator(num_components, getExtrapolator(), local_assemblers_, std::move(f)));
    };
...
void RichardsMechanicsProcess<DisplacementDim>::initializeBoundaryConditions(std::map<int, std::shared_ptr<MaterialPropertyLib::Medium>> const& media)
{
    if (_use_monolithic_scheme)
    {
        initializeProcessBoundaryConditionsAndSourceTerms(*_local_to_global_index_map, 0, media);
        return;
    }
...
template <int DisplacementDim>
void RichardsMechanicsProcess<DisplacementDim>::assembleConcreteProcess(double const t, double const dt, std::vector<GlobalVector*> const& x, std::vector<GlobalVector*> const& x_prev, int const process_id, GlobalMatrix& M, GlobalMatrix& K, GlobalVector& b)
{
    AssemblyMixin<RichardsMechanicsProcess<DisplacementDim>>::assemble(t, dt, x, x_prev, process_id, M, K, b);
}

template <int DisplacementDim>
void RichardsMechanicsProcess<DisplacementDim>::assembleWithJacobianConcreteProcess(double const t, double const dt, std::vector<GlobalVector*> const& x, std::vector<GlobalVector*> const& x_prev, int const process_id, GlobalVector& b, GlobalMatrix& Jac)
{
...
template <int DisplacementDim>
void RichardsMechanicsProcess<DisplacementDim>::preTimestepConcreteProcess(std::vector<GlobalVector*> const& x, double const t, double const dt, int const process_id)
{
...
template <int DisplacementDim>
void RichardsMechanicsProcess<DisplacementDim>::postTimestepConcreteProcess(std::vector<GlobalVector*> const& x, std::vector<GlobalVector*> const& x_prev, double const t, double const dt, int const process_id)
{
...
std::vector<std::vector<std::string>> RichardsMechanicsProcess<DisplacementDim>::initializeAssemblyOnSubmeshes(std::vector<std::reference_wrapper<MeshLib::Mesh>> const& meshes)
{
    ...
    AssemblyMixin<RichardsMechanicsProcess<DisplacementDim>>::initializeAssemblyOnSubmeshes(meshes, names);
    return names;
}

template <int DisplacementDim>
void RichardsMechanicsProcess<DisplacementDim>::computeSecondaryVariableConcrete(double const t, double const dt, std::vector<GlobalVector*> const& x, GlobalVector const& x_prev, int const process_id)
{
    if (process_id != 0) return;
...
template <int DisplacementDim>
NumLib::LocalToGlobalIndexMap const& RichardsMechanicsProcess<DisplacementDim>::getDOFTable(int const process_id) const
{
    if (hasMechanicalProcess(process_id)) return *_local_to_global_index_map;
