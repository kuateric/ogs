...
def parse_records(log_text: str) -> list[dict[str, float | str]]:
    ...
    for match in RUNTIME_RE.finditer(log_text):
        d = match.groupdict()
        records.append({"pair": d["pair"], "gap": float(d["gap"]),
                        "slip": float(d["slip"]), "tn": float(d["tn"]),
                        "tt": float(d["tt"]), "state": d["state"],
                        "ps": float(d["ps"])})
    return records
...
def validate(records: list[dict[str, float | str]]) -> dict[str, float | str]:
    ...
    y_tn, y_tt = float(yielded["tn"]), float(yielded["tt"])
    y_slip, y_ps = float(yielded["slip"]), float(yielded["ps"])
    assert_close(abs(y_tt), MU * abs(y_tn), rel=2e-6, abs_=1e-6)
    assert_close(y_ps, y_slip - y_tt / K_T, rel=2e-6, abs_=1e-8)
    if abs(y_ps) <= 1e-6:
        raise AssertionError("SLIP event did not advance plastic slip")

    last = records[-1]
    gap, slip = float(last["gap"]), float(last["slip"])
    tn, tt, ps = float(last["tn"]), float(last["tt"]), float(last["ps"])
    ...
    assert_close(ps, slip - tt / K_T, rel=2e-6, abs_=1e-8)
    assert_close(ps, y_ps, rel=2e-6, abs_=1e-8)
    util = abs(tt) / expected_limit if expected_limit else math.inf
    assert_close(util, 1.0, rel=2e-6, abs_=2e-6)
    return {**last, "yield_state": yielded["state"],
            "friction_limit": expected_limit, "friction_utilization": util}
...
def main() -> int:
    ...
    cmd = [str(args.ogs), "-o", str(result_dir), "-m",
           str(args.source_prj.parent), str(runtime_prj)]
    ...
    print("G5 two-segment/tubbing joint benchmark: PASS — "
