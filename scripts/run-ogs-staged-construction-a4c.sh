...
stages='r0 r2b r2c r2d r2e r2f r2g r2h r2i r2j r2k r2k-f01 r2l r3a r3b r3c r3d r3e r3f r3g r3h r3i a0 a1 a3 a4 a4b a4c a4d a4e a4f'
...
if [ "$rc" -ne 0 ]; then
    echo "A4F strong-contrast run failed with rc=$rc" >&2
    exit "$rc"
fi
...
if not times or max(times) < 8.0 - 1e-12:
    raise RuntimeError(f'A4F full backfill horizon not reached; max={max(times) if times else None}')
...
if completed < 2:
    raise RuntimeError(f'A4F expected removal and activation continuations; completed={completed}')
...
if not re.search(r'Staged construction trial for process.*?unchanged physical time t\s*=\s*1\.2', log):
    raise RuntimeError('A4F activation continuation after pre-solve trial at t=1.2 not observed')
Path('staged-a4c-evidence.txt').write_text(
    f'upstream_sha=adf770974c7ee0435702fe617634d03d17ab7cb8\n'
    f'gate=A4F_strong_contrast_zero_birth_history_e2e\n'
    ...
    f'constitutive_previous_state_at_birth=zero_relative_to_placement\n'
