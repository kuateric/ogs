...
# a constitutive kinematic homotopy. A4L, applied after A4J in the authoritative
# stack, deliberately removes this experimental homotopy again and restores the
# literature-backed full-physics stress-free-birth operator.
...
# pre-solve activation path. A4J adds the pending-publication API later in the
# patch sequence; the final translation unit sees that API.
...
solve_replacement = '''    // A4K: birth must precede physical advancement of an unsupported cavity.
    // Publish from the last converged placement configuration before the
    // ordinary physical solve. A4L later restores a full-physics birth operator.
...
# as its own explicit stage immediately after A4I. A4L must follow A4J because
# A4J creates the deferred publication path that A4L converts to full-physics
# stress-free birth.
