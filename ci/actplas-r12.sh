...
root=pathlib.Path.cwd(); src=root/'Tests/Data/Mechanics/Excavation'; dst=root/'actplas-cases'/'SM_MC_C5M_RDT_DIAG'
...
(root/'actplas-evidence'/'generated-prj'/'SM_MC_C5M_RDT_DIAG.prj').write_text(text,encoding='latin-1')
PY

# Diagnostic-only source instrumentation: do not alter constitutive/deactivation mechanics.
# Preserve the exact upstream source and only enrich the thrown diagnostic with MGIS rdt.
...
cmake --build "$BUILD_DIR" --target ogs --parallel 2 > actplas-evidence/logs/r12_rebuild.log 2>&1

dir="actplas-cases/SM_MC_C5M_RDT_DIAG"
outdir="$(pwd)/actplas-evidence/out_RDT_DIAG"; mkdir -p "$outdir"
log="$(pwd)/actplas-evidence/logs/RDT_DIAG.log"
set +e
...
mdt="$(printf '%s' "$msg" | sed -nE 's/.*dt=([^\. ]+(\.[0-9]+)?).*/\1/p')"; [[ -n "$mdt" ]] || mdt='-'
...
printf 'case\texit_code\tcompleted\tmfront_failure\tmgis_rdt\tglobal_dt\tfirst_failed_time\tlast_nonlinear_iteration\n' > actplas-evidence/sm-r12.tsv
printf 'RDT_DIAG\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$rc" "$completed" "$mf" "$rdt" "$mdt" "$ft" "$li" | tee -a actplas-evidence/sm-r12.tsv
...
printf '%s\n' 'R12 diagnostic-only instrumentation: exact upstream OGS mechanics are preserved; MFrontGeneric exception text is temporarily enriched to expose MGIS behaviour_data.rdt and dt after status != 1.' > actplas-evidence/r12-purpose.txt
cat actplas-evidence/sm-r12.tsv
